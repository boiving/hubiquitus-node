...
hGetThreads.prototype.exec = function(hMessage, context, cb){
    db = require('../dbPool.js').db.getDb(validator.getDomainJID(hMessage.actor));
...
hGetThreads.prototype.filterMessages = function(actor, convids, context, cb){
    ...
    var stream = db.get(actor).find({convid: new RegExp(regexConvids), type: {$ne: "hConvState"}}).stream();
